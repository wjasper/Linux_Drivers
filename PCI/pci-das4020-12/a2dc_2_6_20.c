...
static int __devinit das4020_init_one( struct pci_dev *pdev, const struct pci_device_id *ent )
{
  ...
  for ( i = 0; i < DA_CHANNELS; i++ ) {
    ...
    class_device_create(das4020_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  }
  ...
  */
  ...
  printk("%s: IRQ=%d 8/29/2007 Warren J. Jasper <wjasper@tx.ncsu.edu>\n",
	 ADAPTER_ID, BoardData[NumBoards].irq);
...
static void das4020_remove_one( struct pci_dev *pdev )
{
  ...
  for (i = 0; i < AD_CHANNELS + DA_CHANNELS + DIO_PORTS; i++) {
    minor = (NumBoards<<0x4) + i;
    class_device_destroy(das4020_class, MKDEV(MajorNumber, minor));
  }
...
/* Reads the calibration coeffs (gain and offset) from the EEPROM  and stroers it in cal_data array */
static void store_calibration_coef( BoardRec *boardData ) 
{
  int i;
