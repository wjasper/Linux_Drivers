/***************************************************************************
 Copyright (C) 2007-2015  Warren Jasper <wjasper@ncsu.edu>
 All rights reserved.
...
static long das08_ioctl(struct file *filePtr, unsigned int cmd, unsigned long arg);
static irqreturn_t das08_ReadInterrupt(int irq, void *dev_id);
static void das08_TimerHandler(struct timer_list *timer);
...
static struct pci_device_id das08_id_tbl[] = {
  {
...
static int das08_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
  ...
  if (request_irq(pdev->irq, das08_ReadInterrupt, (IRQF_SHARED),
  ...
  for (i = 0; i < AD_CHANNELS; i++) {
    ...
    sprintf(name, "das08/ad%d_%d", NumBoards, i);
    device_create(das08_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  }
  ...
  */
  ...
  BoardData[NumBoards].tmd->data = &BoardData[NumBoards];
  timer_setup(&BoardData[NumBoards].tmd->timer, das08_TimerHandler, 0);
...
static void das08_TimerHandler(struct timer_list *t)
{
  struct timer_data *tmd = from_timer(tmd, t, timer);
  BoardRec *boardRecPtr = (BoardRec *) tmd->data;
  /* 
...
static int TrigRead(ADC_ChanRec *chanRec, BoardRec* boardRecPtr, struct file *readfile)
{
  ...
  mod_timer(&(boardRecPtr->tmd->timer), jiffies + HZ);
