/***************************************************************************
 Copyright (C) 2003-2007  Warren J. Jasper
 All rights reserved.
...
static int __devinit das1000_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
  ...
  if (request_irq(pdev->irq, das1000_Interrupt, (IRQF_DISABLED | IRQF_SHARED),
      "das1000", (void *) &BoardData[NumBoards])) {
  ...
  for (i = 0; i < AD_CHANNELS; i++) {
    ...
    device_create(das1000_class, NULL, MKDEV(MajorNumber, minor), name);
  }
