...
#device = []
#device.append(mccEthernetDevice(0x138, '192.168.0.101'))
...
while(True):
  ...
  if ch == 'b':
    ...
    if(etc32.Status == 1):  # EXP detected
      (date_base, date_exp) = etc32.CalDate_R()
      print('Calibration Date (base): ', date_base)
      print('Calibration Date (exp): ', date_exp)
    else:
      date_base = etc32.CalDate_R()
      print('Calibration Date (base): ', date_base)
    print('')
    etc32.FactoryCoef_R()
