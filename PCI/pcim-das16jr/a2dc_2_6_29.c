...
static int das16jr_ioctl(struct inode *iNode, struct file *filePtr, unsigned int cmd, unsigned long arg);
...
static struct pci_device_id das16jr_id_tbl[] __devinitdata = {
  {
...
static int __devinit das16jr_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
  int i;
  int minor;
  u32 base0;
  ...
  if (request_irq(pdev->irq, das16jr_Interrupt, (IRQF_DISABLED | IRQF_SHARED),
      "pcim_das16jr", (void *) &BoardData[NumBoards])) {
  ...
  */
  ...
  printk("%s: IRQ=%d 9/1/2011 wjasper@ncsu.edu\n", ADAPTER_ID, BoardData[NumBoards].irq);
