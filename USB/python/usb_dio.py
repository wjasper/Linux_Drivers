...
class usb_1024LS(usb_dio):
  def __init__(self, serial=None):
    usb_dio.__init__(self)
    try:
...
class usb_DIO24(usb_dio):
  def __init__(self, serial=None):
    usb_dio.__init__(self)
    try:
      self.h.open(0x09db, 0x0093, serial)           # MCC USB-1024LS
    except:
...
class usb_DIO24H(usb_dio):
  def __init__(self, serial=None):
    usb_dio.__init__(self)
    try:
      self.h.open(0x09db, 0x0094, serial)           # MCC USB-1024LS
    except:
      print('Can not open USB-DIO24H')
      return

    self.productID = 0x0094
