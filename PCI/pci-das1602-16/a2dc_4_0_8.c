/***************************************************************************
 Copyright (C) 1997 - 2015  Warren J. Jasper    <wjasper@tx.ncsu.edu>
 All rights reserved.
...
static struct pci_device_id das1602_id_tbl[] = {
  ...
  .device     =   PCI_DEVICE_ID_CBOARDS_DAS1602_16,
...
static struct pci_driver das1602_driver = {
  .name     =   "das1602",
...
static int __init das1602_init(void) 
{
 ...
 /* Register as a device with kernel.  */
  if (MajorNumber) {
    ...
    err = register_chrdev_region(dev, 0xff, "das1602");
  } else {
    err = alloc_chrdev_region(&dev, 0, 0xff, "das1602");
  ...
  /* create the class for the das1602 */
  das1602_class = class_create(THIS_MODULE, "das1602-16");
  if (IS_ERR(das1602_class)) {
    return PTR_ERR(das1602_class);
  }

  err = pci_register_driver(&das1602_driver);
  return  err;	
}
...
static int das1602_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
  ...
  base0 = BoardData.base0 = (u16) pci_resource_start(pdev, 0);

  /* Get PCI_BASE_ADDRESS_1 */
  base1 = BoardData.base1 = (u16) pci_resource_start(pdev, 1);

  /* Get PCI_BASE_ADDRESS_2 */
  base2 = BoardData.base2 = (u16) pci_resource_start(pdev, 2);
      
  /* Get PCI_BASE_ADDRESS_3 */
  base3 = BoardData.base3 = (u16) pci_resource_start(pdev, 3);

  /* Get PCI_BASE_ADDRESS_4 */
  base4 = BoardData.base4 = (u16) pci_resource_start(pdev, 4);
  ...
  BoardData.irq = pdev->irq;
  if (request_irq(pdev->irq, das1602_Interrupt, (IRQF_SHARED), "das1602/16", (void *) &BoardData)) {
  ...
  for (i = 0; i < AD_CHANNELS; i++) {
    ...
    sprintf(name, "das1602-16/ad%d_%d", NumBoards, i);
  ...
  for (i = 0; i < DA_CHANNELS; i++) {
    ...
    sprintf(name, "das1602-16/da%d_%d", NumBoards, i);
    device_create(das1602_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  }
  ...
  sprintf(name, "das1602-16/dio%d_0A", NumBoards);
  device_create(das1602_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  minor++;
  sprintf(name, "das1602-16/dio%d_0B", NumBoards);
  device_create(das1602_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  minor++;
  sprintf(name, "das1602-16/dio%d_0C", NumBoards);
  device_create(das1602_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
 ...
 err_out_2:
  free_irq(pdev->irq, (void *) &BoardData);
...
void __exit das1602_exit(void) 
{
  ...
  dev = MKDEV(MajorNumber, 0);
  pci_unregister_driver(&das1602_driver);
...
static void das1602_remove_one(struct pci_dev *pdev)
{
  ...
  free_irq(BoardData.irq, (void *) &BoardData);
        ...
        /* PreTrigger */
         if ( ChanADC[minor].pretrigCount ) {
           if ( count < ChanADC[minor].pretrigCount + PACKETSIZE ) {
		    ...
		    (int) count, ChanADC[minor].pretrigCount, PACKETSIZE);
             return (-1);
	   }
	 }
...
static long das1602_ioctl(struct file *filePtr, unsigned int cmd, unsigned long arg)
{
  int minor = iminor(filePtr->f_path.dentry->d_inode);
  int port;
  int channel;
  int err = 0;
  int size = _IOC_SIZE(cmd);       /* the size bitfield in cmd */
  u16 base1 = BoardData.base1;
  u16 base3 = BoardData.base3;
...
static void LoadADCPacer(BoardRec *board)
{
  ...
  /* Write the values of ctr1 and ctr2 into counter A1 and A2 */
      ...
      printk("LoadADCPacer: freq = %ld   load values: ctr1 %#x ctr2 %#x\n", 
	     (long) board->ADC_freq, board->ADC_ctr1, board->ADC_ctr2);
  #endif
...
static int PacerRead(ADC_ChanRec *chanRec)
{
  ...
  */
  ...
  wait_event_interruptible(das1602_16_wait, (WordsToRead==0) != 0);  // Block in wait state 
...
static void SetTrigger(u32 value, int minor)
{
  switch(value) {
    case GATE_NEG_HYS:
      toggleHys();
