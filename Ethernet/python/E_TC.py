...
CDM_SET_MEM_R           = 0x44 # Read network settings memory
CMD_SET_MEM_W           = 0x45 # Write network settings memory
...
CMD_NETWORK_CONF        = 0x54 # Read the current network configuration
...
class E_TC:
  ...
  def FactoryCoefficients_R(self):
    ...
    try:
      ...
      raise
      print('FactoryCoefficients_R: timeout error.\n')
      return
    if len(r_buffer) == MSG_HEADER_SIZE + MSG_CHECKSUM_SIZE + replyCount:
      if r_buffer[MSG_INDEX_START] == s_buffer[0]                               and \
         r_buffer[MSG_INDEX_COMMAND] == s_buffer[MSG_INDEX_COMMAND] | MSG_REPLY and \
  ...
  def FactoryCoefficients_W(self):
   ...
   # power on. 
    ...
    try:
      ...
      raise
      print('FactoryCoefficients_W: timeout error.\n')
      return
    if len(r_buffer) == MSG_HEADER_SIZE + MSG_CHECKSUM_SIZE + replyCount:
      if r_buffer[MSG_INDEX_START] == s_buffer[0]                               and \
         r_buffer[MSG_INDEX_COMMAND] == s_buffer[MSG_INDEX_COMMAND] | MSG_REPLY and \
  ...
  def FieldCoefficients_R(self):
    ...
    try:
      ...
      raise
      print('FieldCoefficients_R: timeout error.\n')
      return
    if len(r_buffer) == MSG_HEADER_SIZE + MSG_CHECKSUM_SIZE + replyCount:
      if r_buffer[MSG_INDEX_START] == s_buffer[0]                               and \
         r_buffer[MSG_INDEX_COMMAND] == s_buffer[MSG_INDEX_COMMAND] | MSG_REPLY and \
  ...
  def FieldCoefficients_W(self):
   ...
   # power on. 
    ...
    try:
      ...
      raise
      print('FieldCoefficients_W: timeout error.\n')
      return
    if len(r_buffer) == MSG_HEADER_SIZE + MSG_CHECKSUM_SIZE + replyCount:
      if r_buffer[MSG_INDEX_START] == s_buffer[0]                               and \
         r_buffer[MSG_INDEX_COMMAND] == s_buffer[MSG_INDEX_COMMAND] | MSG_REPLY and \
  ...
  def CJCOffset_R(self):
    ...
    try:
      ...
      raise
      print('CJCOffset_R: timeout error.\n')
      return
    if len(r_buffer) == MSG_HEADER_SIZE + MSG_CHECKSUM_SIZE + replyCount:
      if r_buffer[MSG_INDEX_START] == s_buffer[0]                               and \
         r_buffer[MSG_INDEX_COMMAND] == s_buffer[MSG_INDEX_COMMAND] | MSG_REPLY and \
  ...
  def CJCOffset_W(self):
    ...
    try:
      ...
      raise
      print('CJCOffset_W: timeout error.\n')
      return
    if len(r_buffer) == MSG_HEADER_SIZE + MSG_CHECKSUM_SIZE + replyCount:
      if r_buffer[MSG_INDEX_START] == s_buffer[0]                               and \
         r_buffer[MSG_INDEX_COMMAND] == s_buffer[MSG_INDEX_COMMAND] | MSG_REPLY and \
  ...
  def AlarmStatus_R(self):
    ...
    try:
      ...
      raise
      print('AlarmStatus_R: timeout error.\n')
      return
    if len(r_buffer) == MSG_HEADER_SIZE + MSG_CHECKSUM_SIZE + replyCount:
      if r_buffer[MSG_INDEX_START] == s_buffer[0]                               and \
         r_buffer[MSG_INDEX_COMMAND] == s_buffer[MSG_INDEX_COMMAND] | MSG_REPLY and \
  ...
  def AlarmStatus_W(self):
    ...
    try:
      ...
      raise
      print('AlarmStatus_W: timeout error.\n')
      return
    if len(r_buffer) == MSG_HEADER_SIZE + MSG_CHECKSUM_SIZE + replyCount:
      if r_buffer[MSG_INDEX_START] == s_buffer[0]                               and \
         r_buffer[MSG_INDEX_COMMAND] == s_buffer[MSG_INDEX_COMMAND] | MSG_REPLY and \
  ...
  def SettingsMemory_R(self, address, count):
    ...
    s_buffer[MSG_INDEX_COMMAND]        = CDM_SET_MEM_R
    s_buffer[MSG_INDEX_DATA]           = address & 0xff
    s_buffer[MSG_INDEX_DATA+1]         = (address>>8) & 0xff
    s_buffer[MSG_INDEX_DATA+2]         = count & 0xff
    s_buffer[MSG_INDEX_DATA+3]         = (count>>8) & 0xff
