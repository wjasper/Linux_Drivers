...
static void das1002_Interrupt(int irq, void *dev_id, struct pt_regs *regs);
...
static int __init das1002_init(void) 
{
  ...
  if ((err = register_chrdev(MajorNumber, "pci-das1002", &das1002_fops))) {
    printk(KERN_WARNING"%s: Failure to load module. Major Number = %d error %d\n",
	   ADAPTER_ID, MajorNumber, -err);
    return err; 
  }

  if ((err = pci_module_init(&das1002_driver))) return err;
  return  err;
}
...
static int __devinit das1002_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
  ...
  if (request_irq(pdev->irq, das1002_Interrupt, (SA_INTERRUPT | SA_SHIRQ), 
      "pci-das1002", (void *) &BoardData[NumBoards])) {
...
void __exit das1002_exit(void) 
{
  ...
  if (unregister_chrdev(MajorNumber, "pci-das1002") != 0) {
