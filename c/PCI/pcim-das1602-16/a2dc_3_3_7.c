...
/***************************************************************************
 *
 * a2dc_2_6_29.c for PCIM-DAS1602/16
 *
 ***************************************************************************/
#ifndef __KERNEL__
#define __KERNEL__
...
static struct pci_device_id das1602_id_tbl[] __devinitdata = {
  {
...
static int __devinit das1602_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
  int i;
  int minor;
  u32 base0;
...
static ssize_t das1602_write(struct file *filePtr, const char *buf, size_t count, loff_t *off)
{
  ...
  if ( (chan >= AD_CHANNELS+DIO_PORTS) && (chan < AD_CHANNELS+DIO_PORTS+DA_CHANNELS) ) {
    ...
    if (port == 0)  {  //DAC 0
      outw(ChanDAC[board][port].value, DAC0_DATA_REG);
    } else {           //DAC 1
      outw(ChanDAC[board][1].value, DAC1_DATA_REG);
    }

    #ifdef DEBUG
        printk("das1602_write():  DAC Port = %d, gain = %d\n  value = %#x",
