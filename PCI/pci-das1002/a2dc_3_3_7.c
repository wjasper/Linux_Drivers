/***************************************************************************
 Copyright (C) 2007-2012  Warren J. Jasper    <wjasper@tx.ncsu.edu>
 All rights reserved.
...
static void pci_das1002_AD_HalfFullInt(BoardRec *boardRecPtr);
static void pci_das1002_AD_PretrigInt(BoardRec *boardRecPtr);
static void pci_das1002_AD_NotEmptyInt(BoardRec *boardRecPtr);

static int __init das1002_init(void);
static void __exit das1002_exit (void);
static ssize_t das1002_read(struct file *filePtr, char *buf, size_t count, loff_t *off);
static ssize_t das1002_write(struct file *filePtr, const char *buf, size_t count, loff_t *off);
static int das1002_open(struct inode *iNode, struct file *filePtr);
static int das1002_close(struct inode *iNode, struct file *filePtr);
static long das1002_ioctl(struct file *filePtr, unsigned int cmd, unsigned long arg);
static int das1002_mmap(struct file *filePtr, struct vm_area_struct * vma);
static unsigned int das1002_poll(struct file *filePtr, poll_table *wait);
static int das1002_fault(struct vm_area_struct *vma, struct vm_fault *vmf);
static irqreturn_t das1002_Interrupt(int irq, void *dev_id);

static void calibrate_pci_das1002_aout( int channel, int gain, BoardRec *boardRecPtr);
static void calibrate_pci_das1002_ain( int gain, BoardRec *boardRecPtr);
static u8 get_pci_das1002_nVRam( u16 addr, u32 base0 );
static void write_pci_das1002_8800( u8 addr, u8 value, u32 base1 );
static void write_pci_das1002_7376( u8 value, u32 base1 );

MODULE_AUTHOR("Warren J. Jasper  <wjasper@tx.ncsu.edu>");
MODULE_DESCRIPTION("Driver for the PCI-DAS1002 module");
MODULE_LICENSE("GPL");

module_init(das1002_init);
module_exit(das1002_exit);
...
static DECLARE_WAIT_QUEUE_HEAD(das1002_wait);
static int NumBoards = 0;                            /* number of boards found      */
static spinlock_t das1002_lock;

static struct cdev das1002_cdev;
static struct class *das1002_class;

static struct file_operations das1002_fops = {
  .owner          = THIS_MODULE,
  .read           = das1002_read,
  .write          = das1002_write,
  .unlocked_ioctl = das1002_ioctl,
  .mmap           = das1002_mmap,
  .open           = das1002_open,
  .release        = das1002_close,
  .poll           = das1002_poll,
};

static struct vm_operations_struct das1002_vops = {
  .fault = das1002_fault,
};
...
static int das1002_init_one(struct pci_dev *pdev, const struct pci_device_id *ent);
static void das1002_remove_one(struct pci_dev *pdev);

static struct pci_device_id das1002_id_tbl[] __devinitdata = {
  {
  .vendor     =   PCI_VENDOR_ID_CBOARDS,
  .device     =   PCI_DEVICE_ID_CBOARDS_DAS1002,
...
MODULE_DEVICE_TABLE(pci, das1002_id_tbl);

static struct pci_driver das1002_driver = {
  .name     =   "das1002",
  .id_table =   das1002_id_tbl,
  .probe    =   das1002_init_one,
  .remove   =   das1002_remove_one,
};
...
static int __init das1002_init(void) 
{
 ...
 /* Register as a device with kernel.  */
  if (MajorNumber) {
    ...
    err = register_chrdev_region(dev, 0xff, "das1002");
  } else {
    err = alloc_chrdev_region(&dev, 0, 0xff, "das1002");
  ...
  cdev_init(&das1002_cdev, &das1002_fops);
  das1002_cdev.owner = THIS_MODULE;
  das1002_cdev.ops = &das1002_fops;
  err = cdev_add(&das1002_cdev, dev, 0xff);
  if (err) {
    printk("%s: Error %d in registering file operations", ADAPTER_ID, err);
    return err;
  }

  /* create the class for the das1002 */
  das1002_class = class_create(THIS_MODULE, "das1002");
  if (IS_ERR(das1002_class)) {
    return PTR_ERR(das1002_class);
  }

  err = pci_register_driver(&das1002_driver);
  return  err;	
}

static int __devinit das1002_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
  int i;
  int minor;
  u32 base0;
  ...
  if (NumBoards >= MAX_BOARDS) {
    printk("das1002_init_one: NumBoards = %d. Can't exceed MAX_BOARDS.  edit a2dc.h.\n", NumBoards);
    goto err_out_0;
  }
  ...
  if (request_irq(pdev->irq, das1002_Interrupt, (IRQF_DISABLED | IRQF_SHARED),
      "das1002", (void *) &BoardData[NumBoards])) {
  ...
  /* create a device in /sys/class/das1002/ */
  for (i = 0; i < AD_CHANNELS; i++) {
    minor = (NumBoards<<0x4) + i;
    sprintf(name, "das1002/ad%d_%d", NumBoards, i);
    device_create(das1002_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  }
  ...
  for (i = 0; i < DA_CHANNELS; i++) {
    ...
    sprintf(name, "das1002/da%d_%d", NumBoards, i);
    device_create(das1002_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  }
  ...
  sprintf(name, "das1002/dio%d_0A", NumBoards);
  device_create(das1002_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  minor++;
  sprintf(name, "das1002/dio%d_0B", NumBoards);
  device_create(das1002_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  minor++;
  sprintf(name, "das1002/dio%d_0C", NumBoards);
  device_create(das1002_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  ...
  /* Select unipolar 10V range for both DACS
     ...
     pci1002_aout.  Set BoardData.nSpare3 to zero to
     force calibration.
  */
  ...
  calibrate_pci_das1002_ain( UP_10_00V, &BoardData[NumBoards] );
  ...
  init_waitqueue_head(&das1002_wait);
  spin_lock_init(&das1002_lock);
  ...
  */
  ...
  #ifdef DEBUG
    printk("das1002_init_one: Board %d: buf_phy_size = %#x\n",
	   NumBoards, BoardData[NumBoards].buf_phy_size);
    printk("das1002_init_one: Board %d: buf_virt_addr = %p  buf_bus_addr = %#x\n",
...
/***************************************************************************
 *
 * Remove driver. Called when "rmmod pci-das1002" is run on the command line.
 *
 ***************************************************************************/

void __exit das1002_exit(void) 
{
  dev_t dev;

  dev = MKDEV(MajorNumber, 0);
  pci_unregister_driver(&das1002_driver);
  class_destroy(das1002_class);
  cdev_del(&das1002_cdev);
  unregister_chrdev_region(dev, 0xff);
  printk("%s: module removed from das1002_exit.\n", ADAPTER_ID);
}
...
static void das1002_remove_one(struct pci_dev *pdev)
{
  ...
  for (i = 0; i < AD_CHANNELS + DA_CHANNELS + DIO_PORTS; i++) {
    minor = (NumBoards<<0x4) + i;
    device_destroy(das1002_class, MKDEV(MajorNumber, minor));
  }
...
static int das1002_open(struct inode *iNode, struct file *filePtr)
{
  ...
  */
  ...
  if ( chan >= 0 && chan < AD_CHANNELS ) {
    ...
    wReg = (ADFLCL | INTCL | EOACL);
    spin_lock_irqsave(&das1002_lock, flags);
    BoardData[board].nSpare0 &= ~(EOAIE | INTE);
    spin_unlock_irqrestore(&das1002_lock, flags);
...
static int das1002_close(struct inode *iNode, struct file *filePtr)
{
  ...
  if ( chan >= 0 && chan < AD_CHANNELS ) {
    ...
    printk("das1002_close: Incorrect minor number (%d).\n", minor);
  }  
...
static ssize_t das1002_read(struct file *filePtr, char *buf, size_t count, loff_t *off)
{
  ...
  if ( count < 1 ) {
    printk("das1002_read(): count must be greater than 0.\n");
  ...
  if ( chan >= 0 && chan < AD_CHANNELS ) {
    ...
    if ( (ChanADC[board][chan].count = count) > BoardData[board].buf_phy_size ) {
        printk("das1002_read(): requesting too large a count size.\n");
        BoardData[board].busyRead = FALSE;
        return (-1);
    }
    ...
    switch (ChanADC[board][chan].pacerSource) {
      ...
      case ADC_SOFT_CONVERT:
          #ifdef DEBUG
              printk("das1002_read(): Entering ADC_SOFT_CONVERT mode.  count = %d\n",
		     (int) count);
          #endif

          if (SoftRead(&ChanADC[board][chan], &BoardData[board])) {
              printk("das1002_read: SoftRead() failed.\n");
              BoardData[board].busyRead = FALSE;
              return(-1);
          } 
          break;
      ...
      case ADC_EXTERNAL_PACER_FALLING:
        #ifdef DEBUG
            printk("das1002_read(): Entering ADC_EXTERNAL_PACER_FALLING mode.\n");
        #endif

        /* PreTrigger */
         if ( ChanADC[board][chan].pretrigCount ) {
           if ( count < ChanADC[board][chan].pretrigCount + PACKETSIZE ) {
             printk("das1002_read(): TotalCount (%d) must be greater than PretrigCount (%d) + %d.\n",
         ...
         if (PacerRead(&ChanADC[board][chan], &BoardData[board], filePtr)) {
            printk("das1002_read: PacerRead() failed.\n");
            BoardData[board].busyRead = FALSE;
            return(-1);
	 }
         break;
      ...
      case ADC_EXTERNAL_PACER_RISING:
        #ifdef DEBUG
            printk("das1002_read(): Entering ADC_EXTERNAL_PACER_RISING mode.\n");
        #endif
      ...
      case ADC_PACER_CLOCK:
          #ifdef DEBUG
              printk("das1002_read(): Entering ADC_PACER_CLOCK mode.\n");
          #endif

          if ( ChanADC[board][chan].count == 1 ) {            /* use SoftRead */
              if (SoftRead(&ChanADC[board][chan], &BoardData[board])) {
                  printk("das1002_read: SoftRead() failed with pacer.\n");
                  BoardData[board].busyRead = FALSE;
                  return(-1);
              }
          } else {
  ...
  if ( chan >= AD_CHANNELS && chan < AD_CHANNELS + DIO_PORTS ) {
    ...
    #ifdef DEBUG
       printk("das1002_read: %s DIO Port %#x addr %#x set to %#x\n", 
  ...
  printk("das1002_read: Incorrect minor number (%d).\n", minor);
  BoardData[board].busyRead = FALSE;
  return -1;
}
...
static ssize_t das1002_write(struct file *filePtr, const char *buf, size_t count, loff_t *off)
{
  ...
  #ifdef DEBUG
    printk("das1002_write(): Minor = %d, Count = %d\n", minor, (int) count);
  #endif

  if ( count < 1 ) {
    printk("das1002_write(): count must be greater than 0.\n");
    return (-1);
  }
  ...
  if ( (chan >= AD_CHANNELS) && (chan < AD_CHANNELS+DIO_PORTS) ) {
    ...
    #ifdef DEBUG
    printk("das1002_write %s: DIO Port %#x set to %#x\n", ADAPTER_ID, port, bReg);
    #endif
  ...
  if ( (chan >= AD_CHANNELS+DIO_PORTS) && (chan < AD_CHANNELS+DIO_PORTS+DA_CHANNELS) ) {
    ...
    #ifdef DEBUG
        printk("das1002_write():  port = %d, gain = %d\n  value = %#x",
...
static int das1002_mmap( struct file *filePtr, struct vm_area_struct *vma ) 
{
  ...
  unsigned int minor = iminor(filePtr->f_dentry->d_inode);
  ...
  if (chan >= 0 && chan < AD_CHANNELS) {
    ...
    #ifdef DEBUG
    printk("%s: das1002_mmap(): board %d chan %d.\n", ADAPTER_ID, board, chan);
    printk("das1002_mmap(): vma->vm_start = %#lx  vma->vm_end= %#lx size = %#x offset = %#lx\n",
	   vma->vm_start, vma->vm_end, size, vma->vm_pgoff);
    #endif
    
    if(vma->vm_pgoff != 0) {     // You have to map the entire buffer.
      #ifdef DEBUG
        printk("das1002_mmap(): The page offset has to be zero \n");
      #endif
      return(-EINVAL);
    }

    if (size > BoardData[board].buf_phy_size) {     // You cannot request more than the max buffer
      printk("das1002_mmap(): Size = %d is too large.\n", size);
      return(-EINVAL);
    }

    vma->vm_ops = &das1002_vops;
...
static int das1002_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
  ...
  if (offset >=  BoardData[board].buf_phy_size) {
    printk("das1002_fault: offset = %#lx  buf_phys_size = %#x\n", offset, BoardData[board].buf_phy_size);
  ...
  #ifdef DEBUG
    printk ("das1002_fault mapped address %p to %p\n", vmf->virtual_address, v_addr);
  #endif
  return 0;
}
...
static unsigned int das1002_poll(struct file *filePtr, poll_table *wait)
{
  ...
  /* tell select()/poll() to wake up and give us a call if
     das1002_wait is awakened
  */
  poll_wait(filePtr, &das1002_wait, wait);
...
static irqreturn_t das1002_Interrupt(int irq, void *dev_id)
{
  ...
  /* Check that interrupt came from the pci-das1002 board (needed for sharded interrupts) */
  if (inl(INTCSR_ADDR) & 0x00820000) {
    ...
    #ifdef DEBUG
        printk("Entering das1002_Interrupt(). irq = %d  WordsToRead = %d  IntSource = %#hx\n", 
    ...
    while (IntSource) {
      if (IntSource & EOAI) {         /* End of acquisition          */
        IntSource &= ~EOAI;
        pci_das1002_AD_HalfFullInt(boardRecPtr);
      } else if (IntSource & ADHFI) { /* AD-FIFO-half-full interrupt */
        IntSource &= ~ADHFI;
        if ( boardRecPtr->PreTrigMode ) { 
          pci_das1002_AD_PretrigInt(boardRecPtr);
        } else {
         pci_das1002_AD_HalfFullInt(boardRecPtr);
        }
      } else if (IntSource & ADNEI) { /* AD-not-empty                */
        IntSource &= ~ADNEI;
        pci_das1002_AD_NotEmptyInt(boardRecPtr);
    ...
    if (boardRecPtr->WordsToRead == 0) { 
      ...
      wake_up_interruptible(&das1002_wait);  
    }
  }
  spin_unlock_irqrestore(&das1002_lock, flags);
  return IRQ_HANDLED;
...
/***********************************************************************
 *  This routine gets called from das1002_Interrupt() when the         *
...
static void pci_das1002_AD_HalfFullInt(BoardRec *boardRecPtr)
{
...
#ifdef DEBUG
      printk("Entering pci_das1002_AD_HalfFullInt()\n");
  #endif
  ...
  if (inw(IRQ_REG) & LADFUL) {
    ...
    printk("pci_das1002_AD_HalfFullInt: LADFUL error\n");
    return;
  }
...
static void pci_das1002_AD_PretrigInt(BoardRec *boardRecPtr)
{
  ...
  #ifdef DEBUG
      printk("Entering pci_das1002_AD_PretrigInt()\n");
  #endif
...
#ifdef DEBUG
      printk("Entering pci_das1002_AD_NotEmptyInt()\n");
  #endif

  /* check for FIFO Overflow errors */
  if (inw(IRQ_REG) & LADFUL) {
    printk("pci_das1002_AD_NotEmptyInt: ADC FIFO exceeded full state. Data may be lost\n");
    return;
  }
...
static int SoftRead(ADC_ChanRec *chanRec, BoardRec *boardRecPtr)
{
  ...
  while ( boardRecPtr->WordsToRead ) {
    ...
    /*
       ...
       handler, but the DAS1002 board is too fast, and we get
...
static int SetADCGain( u16 gain, BoardRec *boardRecPtr )
{
 ...
 /* if the gain is different from the one already loaded,
  ...
  if ( gain != (boardRecPtr->nSpare1 & (UNIBIP | GS1 | GS0))) {
    calibrate_pci_das1002_ain(gain, boardRecPtr);
 ...
 */
  ...
  */
  ...
  if (chanRec->nonBlockFlag) {  /* do this one time only */
    ...
    wait_event_interruptible(das1002_wait, (boardRecPtr->WordsToRead==0) != 0);  // Block in wait state 
  }
  ...
  */
  ...
  if ( channel ) {  /* DAC channel 1 */
    if ( (wReg & (DAC1R1 | DAC1R0)) != (boardRecPtr->nSpare3 & (DAC1R1 | DAC1R0))) {
      calibrate_pci_das1002_aout( channel, gain, boardRecPtr );
...
/*************************************************************************** 
 * Reads the calibration coeffs from non-volatile RAM in a PCI-DAS1002     *
 * and writes them to the appropriate trim-DACs associated with the analog * 
 * output sections.                                                        *
 * *************************************************************************/

void calibrate_pci_das1002_aout( int channel, int gain, BoardRec *boardRecPtr )
{
  u32 base0 = boardRecPtr->base0;
  u32 base1 = boardRecPtr->base1;
  u16 addr;
  ...
  #ifdef DEBUG
    printk("calibrate_pci_das1002_aout: channel %d set to gain %#x.\n",
            channel, gain);
  #endif
  ...
  */
  ...
  switch( gain ) {
    ...
    case UP_10_00V:
      ...
      break;
    default:
      printk("pci1002_AOut: unknown gain %d.\n", gain);
      return;
  }
  ...
  if ( channel ) {
    ...
    value = get_pci_das1002_nVRam( addr++, base0 );
    write_pci_das1002_8800( DAC1_COARSE_GAIN, value, base1 );
    value = get_pci_das1002_nVRam( addr++, base0 );
    write_pci_das1002_8800( DAC1_OFFSET, value, base1 );
    value = get_pci_das1002_nVRam( addr++, base0 );
    write_pci_das1002_8800( DAC1_FINE_GAIN, value, base1 );
  } else {
    /* DAC0 */
    value = get_pci_das1002_nVRam( addr++, base0 );
    write_pci_das1002_8800( DAC0_COARSE_GAIN, value, base1 );
    value = get_pci_das1002_nVRam( addr++, base0 );
    write_pci_das1002_8800( DAC0_OFFSET, value, base1 );
    value = get_pci_das1002_nVRam( addr++, base0 );
    write_pci_das1002_8800( DAC0_FINE_GAIN, value, base1 );
  }
}
...
void calibrate_pci_das1002_ain( int gain, BoardRec *boardRecPtr )
{
  u32 base0 = boardRecPtr->base0;
  u32 base1 = boardRecPtr->base1;
  u16 addr;
  ...
  */
  ...
  #ifdef DEBUG
      printk("calibrate_pci_das1002_ain: resetting calibration coefficients. gain = %d\n",
              gain);
  #endif
  ...
  switch( gain ) {
    ...
    case UP_1_25V:
      ...
      break;
    default:
      printk("pci1002_ain: unknown gain %d.\n", gain);
      return;
  }

  value = get_pci_das1002_nVRam(addr++, base0);             /* get ADC Coarse offset */
  #ifdef DEBUG
  printk("ADC coarse offset = %#x  ", value);
  #endif
  write_pci_das1002_8800(ADC_COARSE_OFFSET, value, base1);  /* write DAC Trim coarse offset */

  value = get_pci_das1002_nVRam(addr++, base0);             /* get ADC Fine offset */
  #ifdef DEBUG
    printk("ADC fine offset = %#x  ", value);
  #endif
  write_pci_das1002_8800(ADC_FINE_OFFSET, value, base1);    /* write DAC Trim fine offset */

  value = get_pci_das1002_nVRam(addr, base0);               /* get ADC gain */
  #ifdef DEBUG
    printk("ADC gain = %#x\n", value);
  #endif
  write_pci_das1002_7376(value, base1);                     /* set ADC gain */ 

  udelay(1000);          /* waint 1 milliseconds when changing gains */
}
/*************************************************************************** 
 * Reads a byte from the specified address of the                          *
 * non-volatile RAM on a PCI-DAS1002.                                      *
 ***************************************************************************/

u8 get_pci_das1002_nVRam( u16 addr, u32 base0 )
{
...
void write_pci_das1002_8800( u8 addr, u8 value, u32 base1 )
{
  int i;
...
void write_pci_das1002_7376(u8 value, u32 base1 )
{
  int i;
