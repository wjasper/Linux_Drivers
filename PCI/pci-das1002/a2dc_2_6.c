...
static int das1002_ioctl(struct inode *iNode, struct file *filePtr, unsigned int cmd, unsigned long arg);
...
static struct page *das1002_nopage(struct vm_area_struct *vma, unsigned long address, int *write_access);
...
static int __devinit das1002_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
  ...
  for (i = 0; i < AD_CHANNELS; i++) {
    ...
    device_create(das1002_class, NULL, MKDEV(MajorNumber, minor), name);
  }
  ...
  for (i = 0; i < DA_CHANNELS; i++) {
    ...
    sprintf(name, "da%d_%d", NumBoards, i);
    device_create(das1002_class, NULL, MKDEV(MajorNumber, minor), name);
  }
  ...
  */
  ...
  printk("%s: IRQ=%d 9/5/2007 wjasper@tx.ncsu.edu\n", ADAPTER_ID, BoardData[NumBoards].irq);
...
static struct page *das1002_nopage(struct vm_area_struct *vma, unsigned long address, int *write_access)
{
  ...
  if (offset >=  BoardData[board].buf_phy_size) {
    printk("das1002_nopage: offset = %#lx  buf_phys_size = %#x\n", offset, BoardData[board].buf_phy_size);
  ...
  #ifdef DEBUG
    printk ("das1002_nopage mapped address %08lx to %p\n", address, v_addr);
  #endif
  return (page);
}
