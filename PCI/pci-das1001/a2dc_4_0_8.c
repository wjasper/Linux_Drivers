/***************************************************************************
 Copyright (C) 2003-2015  Warren J. Jasper  <wjasper@tx.ncsu.edu>
 All rights reserved.
...
/***************************************************************************
 *
 * a2dc_3_10_11.c for PCI-DAS1001
 *
 ***************************************************************************/
...
static long das1001_ioctl(struct file *filePtr, unsigned int cmd, unsigned long arg);
...
static int das1001_fault(struct vm_area_struct *vma, struct vm_fault *vmf);
static irqreturn_t das1001_Interrupt(int irq, void *dev_id);
...
static struct cdev das1001_cdev;
static struct class *das1001_class;

static struct file_operations das1001_fops = {
  .owner          = THIS_MODULE,
  .read           = das1001_read,
  .write          = das1001_write,
  .unlocked_ioctl = das1001_ioctl,
  .mmap           = das1001_mmap,
  .open           = das1001_open,
...
static struct pci_device_id das1001_id_tbl[] = {
  {
...
static int __init das1001_init(void) 
{
 ...
 /* Register as a device with kernel.  */
  if (MajorNumber) {
    ...
    err = register_chrdev_region(dev, 0xff, "das1001");
  } else {
    err = alloc_chrdev_region(&dev, 0, 0xff, "das1001");
  ...
  cdev_init(&das1001_cdev, &das1001_fops);
  das1001_cdev.owner = THIS_MODULE;
  das1001_cdev.ops = &das1001_fops;
  err = cdev_add(&das1001_cdev, dev, 0xff);
  if (err) {
    printk("%s: Error %d in registering file operations", ADAPTER_ID, err);
    return err;
  }

  /* create the class for the das1001 */
  das1001_class = class_create(THIS_MODULE, "das1001");
  if (IS_ERR(das1001_class)) {
    return PTR_ERR(das1001_class);
  }

  err = pci_register_driver(&das1001_driver);
  return  err;	
}
...
static int das1001_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
  ...
  if (request_irq(pdev->irq, das1001_Interrupt, (IRQF_SHARED), "pci-das1001", (void *) &BoardData[NumBoards])) {
  ...
  /* create a device in /sys/class/das1001/ */
  for (i = 0; i < AD_CHANNELS; i++) {
    minor = (NumBoards<<0x4) + i;
    sprintf(name, "das1001/ad%d_%d", NumBoards, i);
    device_create(das1001_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  }
  ...
  for (i = 0; i < DA_CHANNELS; i++) {
    ...
    sprintf(name, "das1001/da%d_%d", NumBoards, i);
    device_create(das1001_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  }
  ...
  sprintf(name, "das1001/dio%d_0A", NumBoards);
  device_create(das1001_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  minor++;
  sprintf(name, "das1001/dio%d_0B", NumBoards);
  device_create(das1001_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  minor++;
  sprintf(name, "das1001/dio%d_0C", NumBoards);
  device_create(das1001_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  ...
  */
  ...
  for (i = 0; i < BoardData[NumBoards].buf_phy_size/PAGE_SIZE; i++) {
    ...
    printk("das1001_init_one: Board %d: buf_virt_addr = %p  buf_bus_addr = %#x\n",
	   NumBoards, BoardData[NumBoards].buf_virt_addr, (unsigned int) BoardData[NumBoards].buf_bus_addr);
  #endif
  
  printk("%s: BADR0=%#x BADR1=%#x BADR2=%#x BADR3=%#x BADR4=%#x\n",
          ADAPTER_ID, base0, base1, base2, base3, base4);
  printk("%s: IRQ=%d 9/6/2007 wjasper@tx.ncsu.edu\n", ADAPTER_ID, BoardData[NumBoards].irq);
...
void __exit das1001_exit(void) 
{
  ...
  class_destroy(das1001_class);
  cdev_del(&das1001_cdev);
  unregister_chrdev_region(dev, 0xff);
  printk("%s: module removed from das1001_exit.\n", ADAPTER_ID);
}
...
static void das1001_remove_one(struct pci_dev *pdev)
{
  ...
  for (i = 0; i < AD_CHANNELS + DA_CHANNELS + DIO_PORTS; i++) {
    minor = (NumBoards<<0x4) + i;
    device_destroy(das1001_class, MKDEV(MajorNumber, minor));
  }
...
static int das1001_open(struct inode *iNode, struct file *filePtr)
{
  ...
  */
  ...
  if ( chan >= 0 && chan < AD_CHANNELS ) {
    ...
    wReg = (ADFLCL | INTCL | EOACL);
    spin_lock_irqsave(&das1001_lock, flags);
    BoardData[board].nSpare0 &= ~(EOAIE | INTE);
    spin_unlock_irqrestore(&das1001_lock, flags);
...
static int das1001_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
  ...
  if (offset >=  BoardData[board].buf_phy_size) {
    printk("das1001_fault: offset = %#lx  buf_phys_size = %#x\n", offset, BoardData[board].buf_phy_size);
  ...
  #ifdef DEBUG
    printk ("das1001_fault mapped address %p to %p\n", vmf->virtual_address, v_addr);
  #endif
  return 0;
}
