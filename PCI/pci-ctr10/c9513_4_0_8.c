...
static long ctr10_ioctl(struct file *filePtr, unsigned int cmd, unsigned long arg);
static void ctr10_TimerHandler(unsigned long unused);
static irqreturn_t ctr10_Interrupt(int irq, void *dev_id);
 ...
 *
 ***************************************************************************/
static struct cdev ctr10_cdev;
static struct class *ctr10_class;
...
static struct pci_device_id ctr10_id_tbl[] = {
  {
...
static int __init ctr10_init(void) 
{
  ...
  if (MajorNumber) {
    ...
    err = register_chrdev_region(dev, 0xff, "ctr10");
  } else {
    err = alloc_chrdev_region(&dev, 0, 0xff, "ctr10");
  ...
  cdev_init(&ctr10_cdev, &ctr10_fops);
  ctr10_cdev.owner = THIS_MODULE;
  ctr10_cdev.ops = &ctr10_fops;
  err = cdev_add(&ctr10_cdev, dev, 0xff);
  if (err) {
    printk("%s: Error %d in registering file operations", ADAPTER_ID, err);
    return err;
  }

  /* create the class for the ctr10 */
  ctr10_class = class_create(THIS_MODULE, "ctr10");
  if (IS_ERR(ctr10_class)) {
    return PTR_ERR(ctr10_class);
  }

  err = pci_register_driver(&ctr10_driver);
  return  err;	
}
...
static int ctr10_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
  ...
  if (request_irq(pdev->irq, ctr10_Interrupt, (IRQF_SHARED), 
		  "ctr10", (void *) &BoardData[NumBoards])) {
  ...
  for (i = 1; i <= NCOUNTERS; i++) {
    for (j = 0; j < NCHIP; j++) {
      ...
      sprintf(name, "ctr10/ctr%d_%d%d", NumBoards, j, i);
      device_create(ctr10_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
    }
  }
  ...
  minor = (NumBoards<<0x8) + 0;
  sprintf(name, "ctr10/dio%d_0A", NumBoards);
  device_create(ctr10_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  minor = (NumBoards<<0x8) + 8;
  sprintf(name, "ctr10/dio%d_0B", NumBoards);
  device_create(ctr10_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
...
void __exit ctr10_exit(void)
{
  ...
  class_destroy(ctr10_class);
  cdev_del(&ctr10_cdev);
  unregister_chrdev_region(dev, 0xff);
  printk("%s: module removed from ctr10_exit.\n", ADAPTER_ID);
}
...
static void ctr10_remove_one(struct pci_dev *pdev)
{
  ...
  for (i = 0; i <= NCOUNTERS ; i++) {
    for (j = 0; j < NCHIP; j++) {
       minor = (NumBoards << 0x8) | (j << 0x3) | i;
       device_destroy(ctr10_class, MKDEV(MajorNumber, minor));
    }
  }
...
static int ctr10_open(struct inode *iNode, struct file *filePtr)
{
  ...
  */
  ...
  if ((minor & 0x7) == 0) {              /* DIO port */
    ...
    if (Counter[board][chip][counter].mode == CTR10_FREQUENCY) {
...
static ssize_t ctr10_read(struct file *filePtr, char *buf, size_t count, loff_t *off)
{
  ...
  if (counter > 0 && counter <= NCOUNTERS) {  
    switch (chip) {
      case 0x0:
        #ifdef DEBUG
        printk("ctr10_read Counter A: minor = %d counter = %d\n", minor, counter);
        #endif
        cmd_reg = CMD_REG_A;
        data_reg = DATA_REG_A;
	break;
      case 0x1:
        #ifdef DEBUG
        printk("ctr10_read Counter B: minor = %d counter = %d\n", minor, counter);
        #endif
        cmd_reg = CMD_REG_B;
        data_reg = DATA_REG_B;
	break;
...
static ssize_t ctr10_write(struct file *filePtr, const char *buf, size_t count, loff_t *off)
{
...
#ifdef DEBUG
  printk("ctr10_write: minor = %d  counter = %d\n", minor, counter);
#endif
  ...
  if (counter > 0 && counter <= NCOUNTERS) {  
    ...
    switch (Counter[board][chip][counter].mode) {
      case CTR10_COUNTER:              // set mode D for Count #counter 
      ...
      case CTR10_FREQUENCY:
        bReg = 0x1 << (counter-1);
        #ifdef DEBUG
  	  printk("ctr10_write: CTR10_FREQUENCY counter = %d\n", counter);
        #endif
