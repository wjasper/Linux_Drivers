 ...
 *
 *   Documentation:
 *       PCIM-DAS1602/16  User's Manual Revision 5, June 2006
 *       Register Map for the PCIM-DAS1602/16
 *
...
/***************************************************************************
 *
 * a2dc_3_10_11.c for PCIM-DAS1602/16
 *
 ***************************************************************************/
#ifndef __KERNEL__
#define __KERNEL__
...
static void pcim_das1602_AD_HalfFullInt(BoardRec *boardRecPtr);
static void pcim_das1602_AD_NotEmptyInt(BoardRec *boardRecPtr);
static void pcim_das1602_AD_EndOfAcquisitionInt(BoardRec *boardRecPtr);
...
static long das1602_ioctl(struct file *filePtr, unsigned int cmd, unsigned long arg);
static int das1602_mmap(struct file *filePtr, struct vm_area_struct * vma);
static unsigned int das1602_poll(struct file *filePtr, poll_table *wait);
static vm_fault_t das1602_fault(struct vm_fault *vmf);
...
static DECLARE_WAIT_QUEUE_HEAD(das1602_wait);
static int NumBoards = 0;                            /* number of boards found      */
static spinlock_t das1602_lock;

static struct cdev das1602_cdev;
static struct class *das1602_class;

static struct file_operations das1602_fops = {
  .owner          = THIS_MODULE,
  .read           = das1602_read,
  .write          = das1602_write,
  .unlocked_ioctl = das1602_ioctl,
  .mmap           = das1602_mmap,
  .open           = das1602_open,
...
static struct vm_operations_struct das1602_vops = {
    .fault = das1602_fault,
};
...
static int das1602_init_one(struct pci_dev *pdev, const struct pci_device_id *ent);
static void das1602_remove_one(struct pci_dev *pdev);

static struct pci_device_id das1602_id_tbl[] = {
  {
  .vendor     =   PCI_VENDOR_ID_CBOARDS,
  .device     =   PCI_DEVICE_ID_CBOARDS_PCIM_DAS1602_16,
  ...
  {
  .vendor     =   PCI_VENDOR_ID_CBOARDS,
  .device     =   PCI_DEVICE_ID_CBOARDS_PCIE_DAS1602_16,
  .subvendor  =   PCI_ANY_ID,
  .subdevice  =   PCI_ANY_ID,
  .class      =   0,
  .class_mask =   0,
...
MODULE_DEVICE_TABLE(pci, das1602_id_tbl);

static struct pci_driver pcim_das1602_driver = {
  .name     =   "pcim_das1602",
  .id_table =   das1602_id_tbl,
  .probe    =   das1602_init_one,
  .remove   =   das1602_remove_one,
};
...
static int __init das1602_init(void) 
{
 ...
 /* Register as a device with kernel.  */
  if (MajorNumber) {
    ...
    err = register_chrdev_region(dev, 0xff, "pcim_das1602");
  } else {
    err = alloc_chrdev_region(&dev, 0, 0xff, "pcim_das1602");
  ...
  cdev_init(&das1602_cdev, &das1602_fops);
  das1602_cdev.owner = THIS_MODULE;
  das1602_cdev.ops = &das1602_fops;
  err = cdev_add(&das1602_cdev, dev, 0xff);
  if (err) {
    printk("%s: Error %d in registering file operations", ADAPTER_ID, err);
    return err;
  }

  /* create the class for the pcim-das1602/16 */
  das1602_class = class_create(THIS_MODULE, "pcim-das1602-16");
  if (IS_ERR(das1602_class)) {
    return PTR_ERR(das1602_class);
  }

  err = pci_register_driver(&pcim_das1602_driver);
  return  err;	
}
...
static int das1602_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
  ...
  if (NumBoards >= MAX_BOARDS) {
    printk("das1602_init_one: NumBoards = %d. Can't exceed MAX_BOARDS.  edit a2dc.h.\n", NumBoards);
    goto err_out_0;
  }
  ...
  /* Disable Burst Mode and Conversions BADR3 + 6*/
  BoardData[NumBoards].nSpare2 = 0x0;
  outb(BoardData[NumBoards].nSpare2, BURST_CNTL_REG);
  
  /* Register interrupt handler. */
  BoardData[NumBoards].irq = pdev->irq;
  if (request_irq(pdev->irq, das1602_Interrupt, (IRQF_SHARED), "pcim_das1602_16", (void *) &BoardData[NumBoards])) {
  ...
  if (BoardData[NumBoards].nSpare5 & CLK) {
    ...
    printk("das1602_init_one: Pacer clock set to 10 MHz\n");
  } else {
    BoardData[NumBoards].PacerClock = 1000000;
    printk("das1602_init_one: Pacer clock set to 1 MHz\n");
  }

  if (BoardData[NumBoards].nSpare5 & MUX) {
    BoardData[NumBoards].nChannels = 16;
    printk("das1602_init_one: Analog input modes set to 16 single-ended.\n");
  } else {
    BoardData[NumBoards].nChannels = 8;
    printk("das1602_init_one: Analog input modes set to 8 differential.\n");
  }
  ...
  for (i = 0; i < AD_CHANNELS; i++) {
    ...
    if (BoardData[NumBoards].nSpare5 & U_B) {
      ...
      ChanADC[NumBoards][i].gain = BP_10_00V;  // set range to +/-10V
    }
    ChanADC[NumBoards][i].burstMode = FALSE;
  }

  // create a device in /sys/class/das1602/
  for (i = 0; i < BoardData[NumBoards].nChannels; i++) {
    minor = (NumBoards<<0x5) + i;
    sprintf(name, "pcim-das1602-16/ad%d_%d", NumBoards, i);
  ...
  minor = (NumBoards<<0x5) + AD_CHANNELS;
  sprintf(name, "pcim-das1602-16/dio%d_A", NumBoards);
  device_create(das1602_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  minor++;
  sprintf(name, "pcim-das1602-16/dio%d_B", NumBoards);
  device_create(das1602_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  minor++;
  sprintf(name, "pcim-das1602-16/dio%d_C", NumBoards);
  device_create(das1602_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  ...
  for (i = 0; i < DA_CHANNELS; i++) {
    minor = (NumBoards<<0x5) + i + DIO_PORTS + AD_CHANNELS;
    sprintf(name, "pcim-das1602-16/da%d_%d", NumBoards, i);
    device_create(das1602_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  }
  ...
  init_waitqueue_head(&das1602_wait);
  ...
  */
  ...
  #ifdef DEBUG
    printk("das1602_init_one: Board %d: buf_phy_size = %#x\n",
	   NumBoards, BoardData[NumBoards].buf_phy_size);
    printk("das1602_init_one: Board %d: buf_virt_addr = %p  buf_bus_addr = %#x\n",
	   NumBoards, BoardData[NumBoards].buf_virt_addr, (int) BoardData[NumBoards].buf_bus_addr);
  #endif

  /* initialize the ring buffer */
  BoardData[NumBoards].rb_size = BoardData[NumBoards].buf_phy_size / sizeof(*BoardData[NumBoards].ringbuf_vaddr);
  BoardData[NumBoards].ringbuf_vaddr = kmalloc(BoardData[NumBoards].buf_phy_size, GFP_KERNEL);
  if (BoardData[NumBoards].ringbuf_vaddr == 0x0) return -ENOMEM;

  BoardData[NumBoards].rb_head = 0;
  BoardData[NumBoards].rb_head = 0;
  BoardData[NumBoards].rb_init = 1;
  ...
  printk("%s: IRQ=%d 5/14/2016 wjasper@ncsu.edu\n", ADAPTER_ID, BoardData[NumBoards].irq);
...
/**********************************************************************************
 *                                                                                *
 * Remove driver. Called when "rmmod pcim_das1602_16" is run on the command line. *
 *                                                                                *
 **********************************************************************************/
...
void __exit das1602_exit(void) 
{
  ...
  dev = MKDEV(MajorNumber, 0);
  pci_unregister_driver(&pcim_das1602_driver);
  class_destroy(das1602_class);
  cdev_del(&das1602_cdev);
  unregister_chrdev_region(dev, 0xff);
  printk("%s: module removed from das1602_exit.\n", ADAPTER_ID);
}
...
static void das1602_remove_one(struct pci_dev *pdev)
{
  ...
  for (i = 0; i < BoardData[NumBoards].nChannels; i++) {
    minor = (NumBoards<<0x5) + i;
    device_destroy(das1602_class, MKDEV(MajorNumber, minor));
  }

  for (i = AD_CHANNELS; i < AD_CHANNELS + DIO_PORTS + DA_CHANNELS; i++) {
    minor = (NumBoards<<0x5) + i;
    device_destroy(das1602_class, MKDEV(MajorNumber, minor));
  }
  ...
  kfree(BoardData[NumBoards].ringbuf_vaddr);
...
static int das1602_open(struct inode *iNode, struct file *filePtr)
{
  ...
  */
  ...
  if ( chan >= 0 && chan < AD_CHANNELS ) {
    ...
    if (BoardData[board].nSpare5 & U_B) {
      ChanADC[board][chan].gain = UP_10_00V;   // set range to 0-10V
    } else {
      ChanADC[board][chan].gain = BP_10_00V;  // set range to +/-10V
    }
    ...
    spin_lock_irqsave(&das1602_lock, flags);
    BoardData[board].nSpare0 = 0x0;
    spin_unlock_irqrestore(&das1602_lock, flags);
...
static int das1602_close(struct inode *iNode, struct file *filePtr)
{
  ...
  if (BoardData[board].ADC_acqtype == ADC_ACQ_CONTINUOUS) {
    BoardData[board].ADC_acqtype = ADC_ACQ_NORMAL;
    StopADCPacer(&BoardData[board]);  // disable pacer if in pacer mode
  }
...
static ssize_t das1602_read(struct file *filePtr, char *buf, size_t count, loff_t *off)
{
  ...
  if (chan >= 0 && chan < AD_CHANNELS) {
    ...
    if ((BoardData[board].ADC_acqtype == ADC_ACQ_NORMAL) ||
	((BoardData[board].ADC_acqtype == ADC_ACQ_CONTINUOUS) && BoardData[board].rb_init)) {
    ...
    switch (ChanADC[board][chan].pacerSource) {
      ...
      case ADC_SOFT_CONVERT:
          ...
          if (SoftRead(&ChanADC[board][chan], &BoardData[board])) {
              printk("das1602_read: SoftRead() failed.\n");
              BoardData[board].busyRead = FALSE;
              return(-1);
          } 
          break;
...
static ssize_t das1602_write(struct file *filePtr, const char *buf, size_t count, loff_t *off)
{
  ...
  #ifdef DEBUG
  printk("das1602_write(): Minor = %d, Board = %d  Channel = %d, Count = %d\n",
  ...
  if ((chan >= AD_CHANNELS) && (chan < AD_CHANNELS+DIO_PORTS)) {
    ...
    outb_p(bReg, ChanDIO[board][port].addr);
  ...
  if ( (chan >= AD_CHANNELS+DIO_PORTS) && (chan < AD_CHANNELS+DIO_PORTS+DA_CHANNELS) ) {
    ...
    if (port == 0)  {  //DAC 0
      get_user(ChanDAC[board][0].value, (u16*)buf);
      outw(ChanDAC[board][0].value, DAC0_DATA_REG);
    } else {           //DAC 1
      get_user(ChanDAC[board][1].value, (u16*)buf);
      outw(ChanDAC[board][1].value, DAC1_DATA_REG);
    }

    #ifdef DEBUG
        printk("das1602_write():  DAC Port = %d, gain = %d  value = %#x\n",
  ...
  if ( chan >= 0 && chan < AD_CHANNELS ) {
    switch (cmd) {
      case ADC_SET_GAINS:
        ...
        #endif
        ChanADC[board][chan].gain &= ~(G1 | G0);
        ChanADC[board][chan].gain |=  (u16) arg & (G1 | G0);
        break;
      case ADC_GET_GAINS:
	...
	#endif
	break;
      case ADC_SET_ACQ_TYPE:
	/* only allow continusous type in PACER_CLOCK mode */
	if ((ChanADC[board][chan].pacerSource == ADC_PACER_CLOCK) && (arg == ADC_ACQ_CONTINUOUS)) {
	  BoardData[board].ADC_acqtype = ADC_ACQ_CONTINUOUS;
	  BoardData[board].rb_init = 1;
	  BoardData[board].rb_head = 0;
	  BoardData[board].rb_tail = 0;
	} else {
	  BoardData[board].ADC_acqtype = ADC_ACQ_NORMAL;
	}
	#ifdef DEBUG
  	  printk("ioctl: ADC_SET_ACQ_TYPE: type = %d\n", (int) arg);
	#endif
         break;
      case ADC_COUNTER0:
...
static int das1602_mmap( struct file *filePtr, struct vm_area_struct *vma ) 
{
  ...
  if (chan >= 0 && chan < AD_CHANNELS) {
    ...
    #ifdef DEBUG
    printk("%s: pcim-das1602_mmap(): board %d chan %d.\n", ADAPTER_ID, board, chan);
    printk("pcim-das1602_mmap(): vma->vm_start = %#lx  vma->vm_end= %#lx size = %#x offset = %#lx\n",
	   vma->vm_start, vma->vm_end, size, vma->vm_pgoff);
    #endif
    
    if(vma->vm_pgoff != 0) {     // You have to map the entire buffer.
      #ifdef DEBUG
        printk("pcim-das1602_mmap(): The page offset has to be zero \n");
      #endif
      return(-EINVAL);
    }

    if (size > BoardData[board].buf_phy_size) {     // You cannot request more than the max buffer
      printk("pcim-das1602_mmap(): Size = %d is too large.\n", size);
      return(-EINVAL);
    }

    vma->vm_ops = &das1602_vops;
...
static vm_fault_t das1602_fault(struct vm_fault *vmf)
{
  ...
  #ifdef DEBUG
    printk ("pcim-das1602_fault page fault\n");
  #endif
  return 0;
}
...
static unsigned int das1602_poll(struct file *filePtr, poll_table *wait)
{
  ...
  /* tell select()/poll() to wake up and give us a call if
     das1602_wait is awakened
  */
  poll_wait(filePtr, &das1602_wait, wait);
...
static irqreturn_t das1602_Interrupt(int irq, void *dev_id)
{
  ...
  /* Check that interrupt came from the pci-das1602 board (needed for sharded interrupts) */
  if ((intcsr = inl(INTCSR_ADDR)) & PLX_INT) {
    ...
    #ifdef DEBUG
        printk("Entering das1602_Interrupt(). irq = %d  WordsToRead = %d  intSource = %#hx\n", 
              irq, boardRecPtr->WordsToRead, intSource );
    #endif
    if (intSource & EOA) {                      // End of Acaquisition
      pcim_das1602_AD_EndOfAcquisitionInt(boardRecPtr);
    } else if (intSource & FHF) {               // FIFO half full
      pcim_das1602_AD_HalfFullInt(boardRecPtr);
    } else if (intSource & EOB) {               // End of Burst
      pcim_das1602_AD_HalfFullInt(boardRecPtr);
    } else if (intSource & FNE) {               // FIFO Not Empty
      pcim_das1602_AD_NotEmptyInt(boardRecPtr);
    }

    if (boardRecPtr->WordsToRead == 0) {
      if (boardRecPtr->ADC_acqtype == ADC_ACQ_NORMAL) {
	boardRecPtr->nSpare0 &= ~(INTE);
	outb(boardRecPtr->nSpare0, INTCSR_REG);
      }
      wake_up_interruptible(&das1602_wait);  
    }
  }
...
static void pcim_das1602_AD_NotEmptyInt(BoardRec *boardRecPtr)
{
  ...
  #ifdef DEBUG
    printk("Entering pcim_das1602_AD_NotEmptyInt()\n");
  #endif

  /* check for FIFO Overflow errors */
  if (inb(ADC_CONV_REG) & OVERRUN) {
    printk("pcim_das1602_AD_NotEmptyInt: OVERRUN error\n");
    return;
  }
...
static void pcim_das1602_AD_EndOfAcquisitionInt(BoardRec *boardRecPtr)
{
  ...
  #ifdef DEBUG
    printk("Entering pcim_das1602_AD_EndOfAcquisitionInt(). WordsToRead = %d\n",
	   boardRecPtr->WordsToRead);
  #endif
...
static void pcim_das1602_AD_HalfFullInt(BoardRec *boardRecPtr)
{
  ...
  #ifdef DEBUG
    printk("Entering pcim_das1602_AD_HalfFullInt()\n");
  #endif

  /* check for FIFO Overflow errors */
    if (inb(ADC_CONV_REG) & OVERRUN) {
      printk("pcim_das1602_AD_HalfFullInt: OVERRUN error\n");
    ...
    } else {  // handle ADC_ACQ_CONTINUOUS
      for (i = 0; i < PACKETSIZE; i++) {
	boardRecPtr->ringbuf_vaddr[boardRecPtr->rb_head] = inw_p(ADC_DATA_REG); // Load into ring buffer
	boardRecPtr->rb_head = (boardRecPtr->rb_head + 1) % boardRecPtr->rb_size;
      }
      while ((boardRecPtr->WordsToRead) && (boardRecPtr->rb_tail != boardRecPtr->rb_head)) {
	*boardRecPtr->ADC_KernBuffPtr = boardRecPtr->ringbuf_vaddr[boardRecPtr->rb_tail];
	boardRecPtr->rb_tail = (boardRecPtr->rb_tail + 1) % boardRecPtr->rb_size;
	boardRecPtr->ADC_KernBuffPtr++;
	boardRecPtr->WordsToRead--;
      }
      outb((INTE | INTSEL1 | INTSEL0), INTCSR_REG);
      #ifdef DEBUG
        printk("Ringbuff Read only 512 samples.\n");
      #endif
    }
...
static int SoftRead(ADC_ChanRec *chanRec, BoardRec *boardRecPtr)
{
  ...
  unsigned int busy;
  int i;
#define TIMEOUT 1000  // this is likely to be 100 loops on a 2GHz machine, so set to 1000 as the limit
 ...
 /* Disable everything first */
  ...
  outb(CONV_EN, BURST_CNTL_REG); // set burst mode off, enable conversions
  ...
  while (boardRecPtr->WordsToRead) {
    ...
    */
    ...
    for (i = 0; i < TIMEOUT; i++ ) {
      busy = inb_p(ADC_CHAN_REG) & EOC;
      if (!busy) break;
    }
    if (i == TIMEOUT) {
      printk("%s: SoftRead timeout\n", ADAPTER_ID);
      return -ETIMEDOUT;
    }
  ...
  outb(boardRecPtr->nSpare2, BURST_CNTL_REG);  // restore Burst Mode and Converter Control Register
  return 0;
}
 ...
 */
  ...
  if ((boardRecPtr->ADC_acqtype == ADC_ACQ_NORMAL) ||
      ((boardRecPtr->ADC_acqtype == ADC_ACQ_CONTINUOUS) && boardRecPtr->rb_init)) {
  ...
  if ((boardRecPtr->ADC_acqtype == ADC_ACQ_NORMAL) ||
    ...
    boardRecPtr->rb_init = 0;                    // Clear first pass flag
  }
  ...
  if (chanRec->nonBlockFlag) {  /* do this one time only */
    ...
    wait_event_interruptible(das1602_wait, (boardRecPtr->WordsToRead==0) != 0);  // Block in wait state 
  }
