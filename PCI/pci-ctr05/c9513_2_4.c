...
static int ctr05_ioctl(struct inode *iNode, struct file *filePtr, unsigned int cmd, unsigned long arg);
static void ctr05_TimerHandler(unsigned long unused);
static void ctr05_Interrupt(int irq, void *dev_id, struct pt_regs *regs);
...
static int __init ctr05_init(void) 
{
  ...
  if ((err = register_chrdev(MajorNumber, "pci-ctr05", &ctr05_fops))) {
  ...
  err = pci_module_init(&ctr05_driver);	
...
static int __devinit ctr05_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
  ...
  if (request_irq(pdev->irq, ctr05_Interrupt, (SA_INTERRUPT | SA_SHIRQ), 
		  "pci-ctr05", (void *) &BoardData[NumBoards])) {
  ...
  printk(" 1/27/03 wjasper@tx.ncsu.edu\n");
...
void __exit ctr05_exit(void)
{
  ...
  if (unregister_chrdev(MajorNumber, "pci-ctr05") != 0) {
...
static int ctr05_open(struct inode *iNode, struct file *filePtr)
{
  ...
  */
  ...
  #ifdef DEBUG
  printk("%s: open(): minor %d chan %d mode %d\n", ADAPTER_ID, minor, chan,
...
static ssize_t ctr05_read(struct file *filePtr, char *buf, size_t count, loff_t *off)
{
  ...
  if ( chan > 0 && chan <= NCOUNTERS ) {
	...
	data = inb_p(DATA_REG) & 0xff;           /* Low Byte */
        ...
        #ifdef DEBUG
          printk("ctr05_read CTR05_Counter : minor = %d data = %d\n", minor, data);
        #endif
...
#ifdef DEBUG
  printk("ctr05_write: minor = %d chan = %d  mode = %d\n", 
	 minor, chan, Counter[board][chan].mode);
#endif
...
#ifdef DEBUG
	printk("2- ctr05_write: bReg = %d\n", bReg);
#endif

        outb_p (bReg, CMD_REG);

	#ifdef DEBUG
	   printk("write(): CTR05_COUNTER: counter %d loaded and armed\n", chan);
	#endif
        break;
  ...
  if ( chan > 0 && chan <= NCOUNTERS ) {
	...
	outb_p(chan|HOLD_REG, CMD_REG);
      ...
      case GET_MASTER_MODE_REG:
        put_user((long) BoardData[board].mm, (long*) arg);
        break;
      case GET_COUNTER_MODE_REG:
        put_user((long) Counter[board][chan].cm, (long*) arg);
        break;
      case GET_STATUS_REG:
        bReg = inb(CMD_REG);
