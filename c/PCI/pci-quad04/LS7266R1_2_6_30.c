...
static int quad04_ioctl(struct inode *iNode, struct file *filePtr, unsigned int cmd, unsigned long arg);
...
static struct pci_device_id quad04_id_tbl[] __devinitdata = {
  {
...
static int __devinit quad04_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
  int i;
  int minor;
  u16 base1;
  ...
  for (i = 0; i < NCOUNTERS; i++) {
    ...
    sprintf(name, "channel%d_%d", NumBoards, i+1);
