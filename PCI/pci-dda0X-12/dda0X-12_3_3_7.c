...
static int __init dda0X_12_init(void);
static void __exit dda0X_12_exit (void);
static ssize_t dda0X_12_read(struct file *filePtr, char *buf, size_t count, loff_t *off);
static ssize_t dda0X_12_write(struct file *filePtr, const char *buf, size_t count, loff_t *off);
static int dda0X_12_open(struct inode *iNode, struct file *filePtr);
...
module_init(dda0X_12_init);
module_exit(dda0X_12_exit);
 ...
 *
 ***************************************************************************/
static struct cdev dda0X_12_cdev;
static struct class *dda0X_12_class;

static struct file_operations dda0X_12_fops = {
  .owner          = THIS_MODULE,
  .read           = dda0X_12_read,
  .write          = dda0X_12_write,
  .unlocked_ioctl = dda0X_12_ioctl,
  .open           = dda0X_12_open,
  .release        = dda0X_12_close,
};
...
static int dda0X_12_init_one(struct pci_dev *pdev, const struct pci_device_id *ent);
static void dda0X_12_remove_one(struct pci_dev *pdev);

static struct pci_device_id dda0X_12_id_tbl[] __devinitdata = {
  {
  .vendor      =       PCI_VENDOR_ID_CBOARDS,
  .device      =       PCI_DEVICE_ID_CBOARDS_DDA02_12,
  ...
  {
  .vendor      =       PCI_VENDOR_ID_CBOARDS,
  .device      =       PCI_DEVICE_ID_CBOARDS_DDA04_12,
  .subvendor   =       PCI_ANY_ID,
  .subdevice   =       PCI_ANY_ID,
  .class       =       0,
  .class_mask  =       0,
  ...
  {
  .vendor      =       PCI_VENDOR_ID_CBOARDS,
  .device      =       PCI_DEVICE_ID_CBOARDS_DDA08_12,
  .subvendor   =       PCI_ANY_ID,
  .subdevice   =       PCI_ANY_ID,
  .class       =       0,
  .class_mask  =       0,
...
MODULE_DEVICE_TABLE(pci, dda0X_12_id_tbl);

static struct pci_driver dda0X_12_driver = {
  .name     =  "dda0X_12",
  .id_table =  dda0X_12_id_tbl,
  .probe    =  dda0X_12_init_one,
  .remove   =  dda0X_12_remove_one,
};
...
static int __init dda0X_12_init(void) 
{
  ...
  if (MajorNumber) {
    ...
    err = register_chrdev_region(dev, 0xff, "dda0X-12");
  } else {
    err = alloc_chrdev_region(&dev, 0, 0xff, "dda0X-12");
  ...
  cdev_init(&dda0X_12_cdev, &dda0X_12_fops);
  dda0X_12_cdev.owner = THIS_MODULE;
  dda0X_12_cdev.ops = &dda0X_12_fops;
  err = cdev_add(&dda0X_12_cdev, dev, 0xff);
  if (err) {
    printk("%s: Error %d in registering file operations", ADAPTER_ID, err);
    return err;
  }

  /* create the class for the dda0X-12 */
  dda0X_12_class = class_create(THIS_MODULE, "dda0x-12");
  if (IS_ERR(dda0X_12_class)) {
    return PTR_ERR(dda0X_12_class);
  }

  err = pci_register_driver(&dda0X_12_driver);
  return  err;	
}

static int __devinit dda0X_12_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
  int i;
  int minor;
  u16 base2;
  ...
  if (NumBoards >= MAX_BOARDS) {
    printk("dda0X_12_init_one: NumBoards = %d. Can't exceed MAX_BOARDS.  edit dda0X_12.h.\n", NumBoards);
    return -ENODEV;
  }
  ...
  sprintf(name, "dda0x-12/dio%d_0A", NumBoards);
  device_create(dda0X_12_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  minor++;
  sprintf(name, "dda0x-12/dio%d_0B", NumBoards);
  device_create(dda0X_12_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  minor++;
  sprintf(name, "dda0x-12/dio%d_0C", NumBoards);
  device_create(dda0X_12_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  minor++;
  sprintf(name, "dda0x-12/dio%d_1A", NumBoards);
  device_create(dda0X_12_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  minor++;
  sprintf(name, "dda0x-12/dio%d_1B", NumBoards);
  device_create(dda0X_12_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  minor++;
  sprintf(name, "dda0x-12/dio%d_1C", NumBoards);
  device_create(dda0X_12_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  ...
  /* create a device in /sys/class/dda0X-12/ */
  for (i = 0; i < DAC_PORTS; i++) {
    minor = (NumBoards<<0x4) + i;
    sprintf(name, "dda0x-12/da%d_%d", NumBoards, i);
    device_create(dda0X_12_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  }
...
/***************************************************************************
 *
 * Remove driver. Called when "rmmod pci-dda0X_12" is run on the command line.
 *
 ***************************************************************************/

void __exit dda0X_12_exit(void)
{
  dev_t dev;

  dev = MKDEV(MajorNumber, 0);
  pci_unregister_driver(&dda0X_12_driver);
  class_destroy(dda0X_12_class);
  cdev_del(&dda0X_12_cdev);
  unregister_chrdev_region(dev, 0xff);
  printk("%s: module removed from dda0X_12_exit.\n", ADAPTER_ID);
}
...
static void dda0X_12_remove_one(struct pci_dev *pdev)
{
  ...
  for (i = 0; i < DAC_PORTS + DIO_PORTS; i++) {
    minor = (NumBoards<<0x4) + i;
    device_destroy(dda0X_12_class, MKDEV(MajorNumber, minor));
  }
...
static int dda0X_12_open(struct inode *iNode, struct file *filePtr)
{
  ...
  */
  if (port >= 0 && port < 2) {
    ...
    outw(port<<2 | EN, DAC_CNTRL_REG);
    #ifdef DEBUG
      printk("%s: open(): minor %d DAC Channel %d.\n", ADAPTER_ID, minor, port);
    #endif
    return 0;   
  } else if (port >= 2 && port < 4 && ((device == PCI_DEVICE_ID_CBOARDS_DDA04_12) || (device == PCI_DEVICE_ID_CBOARDS_DDA08_12))) {
    if (Chan_DAC[board][port].open == TRUE) {
        return -EBUSY;
    }
    Chan_DAC[board][port].open = TRUE;                 /* The device is open */
    ...
    #endif
    return 0;   
  } else if (port >= 4 && port < 8 && (device == PCI_DEVICE_ID_CBOARDS_DDA08_12)) {
    if (Chan_DAC[board][port].open == TRUE) {
        return -EBUSY;
    }
    Chan_DAC[board][port].open = TRUE;                 /* The device is open */
...
static ssize_t dda0X_12_read(struct file *filePtr, char *buf, size_t count, loff_t *off)
{
  ...
  if (port < DAC_PORTS) {
    ...
    #ifdef DEBUG
    printk("dda0X_12_read: %s DIO Port %#x  address = %#x  value = %#x\n", 
    ...
    printk("dda0X_12_read: Incorrect minor number (%d).\n", minor);
    return -1;
  }
  return 1;
}
...
static ssize_t dda0X_12_write(struct file *filePtr, const char *buf, size_t count, loff_t *off)
{
  ...
  if (port < DAC_PORTS) {
    ...
    #ifdef DEBUG
      printk("dda0X_12_write %s DIO Port %#x  address = %#x  value = %#x\n", 
	     ADAPTER_ID, port, Chan_DIO[board][port].addr, dio_value);
    #endif
    return 1;
  } else {
    printk("dda0X_12_write: Incorrect minor number (%d).\n", minor);
    return -1;
  }
  return 0;
}
