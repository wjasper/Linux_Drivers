...
static int __devinit dio96H_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
  ...
  printk(" 8/30/07 wjasper@tx.ncsu.edu\n");
  ...
  /* create a device in /sys/class/dio96H */
  minor = (NumBoards<<0x4);
  sprintf(name, "dio%d_0A", NumBoards);
  class_device_create(dio96H_class, NULL, MKDEV(MajorNumber, minor), NULL, name);
  minor++;
...
static void dio96H_remove_one(struct pci_dev *pdev)
{
  ...
  for (i = 0; i < DIO_PORTS; i++) {
    ...
    class_device_destroy(dio96H_class, MKDEV(MajorNumber, minor));
  }
