...
static int das1000_ioctl(struct inode *iNode, struct file *filePtr, unsigned int cmd, unsigned long arg);
...
static struct page *das1000_nopage(struct vm_area_struct *vma, unsigned long address, int write_access);
static void das1000_Interrupt(int irq, void *dev_id, struct pt_regs *regs);
...
static struct pci_device_id das1000_id_tbl[] __devinitdata = {
  {
...
static int __init das1000_init(void) 
{
  ...
  if ((err = register_chrdev(MajorNumber, "pci-das1000", &das1000_fops))) {
    printk("KERN_WARNING%s: Failure to load module. Major Number = %d error %d\n",
	   ADAPTER_ID, MajorNumber, -err);
    return err; 
  }

  if ((err = pci_module_init(&das1000_driver))) return err;
  return  err;
}

static int __devinit das1000_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
  int i;
  u32 base0;
  u32 base1;
  ...
  if (request_irq(pdev->irq, das1000_Interrupt, (SA_INTERRUPT | SA_SHIRQ), 
      "pci-das1000", (void *) &BoardData[NumBoards])) {
...
void __exit das1000_exit(void) 
{
  ...
  if (unregister_chrdev(MajorNumber, "pci-das1000") != 0) {
    printk("%s: cleanup module failed.\n", ADAPTER_ID);
  } else {
...
static struct page *das1000_nopage(struct vm_area_struct *vma, unsigned long address, int write_access)
{
  ...
  if (offset >=  BoardData[board].buf_phy_size) {
    printk("das1000_nopage: offset = %#lx  buf_phys_size = %#x\n", offset, BoardData[board].buf_phy_size);
...
#ifdef DEBUG
    printk ("das1000_nopage mapped address %08lx to %p\n", address, v_addr);
  #endif
  return (page);
}
